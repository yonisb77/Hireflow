...
På en kandidats detaljvy kan man klicka "AI-bedöm mot jobbet". Edge-funktionen `assess-candidate` hämtar kandidatens profil (namn, LinkedIn, anteckningar) och jobbets titel/avdelning — via anroparens egen inloggning, så RLS skyddar automatiskt mot att bedöma andras kandidater — skickar det till Claude med en instruktion om att svara med strukturerad JSON (poäng 1–10, sammanfattning, styrkor, svagheter), och sparar resultatet på kandidaten.
